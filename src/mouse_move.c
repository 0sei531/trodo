...
// Handle mouse movement for settings screen
...
// Handle mouse movement for control screen
...
// Handle mouse movement in the game
void mouse_move_game(int x, int y)
{
    ...
    if (data.intro.g_k[0])
    {
        ...
        if (data.mouse.y - y > 1)
            ...
            data.c += 25 * (y - data.mouse.y) / 5 * (data.c + 25 * (y - data.mouse.y) / 5 > 100);
    }
    data.mouse.x = x;
    data.mouse.y = y;
}
...
void handle_events(SDL_Event event)
{
    ...
    else if (event.type == SDL_MOUSEBUTTONDOWN)
    {
        ...
        if (event.button.button == SDL_BUTTON_LEFT)
        {
            ...
            // e.g., setting flags, calling specific functions
        }
    }
}
